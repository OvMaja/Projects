...
OS_TASKENTRY(sendTask) {
	uint8_t msg_id = 0;
	uint8_t msg[4][11];
	uint8_t payload[] = { 0x0A, 0x77 };

	// Create messages
	if (CAN_createFrame(0x5, 0, payload, 2, msg[0], sizeof(msg[0])) < 0)
		printfx("Error creating CAN frame 0! \n");
	if (CAN_createFrame(0x05, 1, payload, 2, msg[1], sizeof(msg[1])) < 0)
		printfx("Error creating CAN frame 1! \n");
	if (CAN_createFrame(0x800, 1, payload, 2, msg[2], sizeof(msg[2])) < 0)
		printfx("Error creating CAN frame 2! \n");
	if (CAN_createFrame(0x04, 0, payload, 2, msg[3], sizeof(msg[3])) < 0)
