...
int8_t 	CAN_receiveFrame(uint8_t* dst, uint8_t dstLen, Time_t time) {
    ...
    if(!Extended){
        ...
        if((TotalBitsReceived == 36) && Extended)
            DLC |= bit << 3;
        if((TotalBitsReceived == 37) && Extended)
            DLC |= bit << 2;
        if((TotalBitsReceived == 38) && Extended)
            DLC |= bit << 1;
        if((TotalBitsReceived == 39) && Extended)
            DLC |= bit;
        if((TotalBitsReceived == (19 + (DLC * 8) +  15)) && !Extended)
            break;
        if((TotalBitsReceived == (39 + (DLC * 8) +  15)) && Extended)
            break;
    }
